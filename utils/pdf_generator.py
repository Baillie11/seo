...
def clean_url_for_filename(url):
    ...
    try:
        ...
        logger.error(f"Error in clean_url_for_filename: {str(e)}")
        # Return a safe default filename if URL cleaning fails
        return f"seo_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
...
def create_report(seo_data, categories):
    ...
    try:
        if not seo_data or not isinstance(seo_data, dict):
            raise ValueError("Invalid SEO data provided")
            
        if not categories or not isinstance(categories, list):
            raise ValueError("Invalid categories provided")
            
        # Create a clean filename from the URL
        filename = clean_url_for_filename(seo_data.get('URL', ''))
        logger.info(f"Generated filename: {filename}")
        
        # Get the absolute path of the current file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Create reports directory path (two levels up from utils)
        ...
        try:
            ...
            logger.info("Reports directory created/verified")
        except Exception as e:
        ...
        logger.info(f"Full report file path: {filepath}")
        
        # Verify write permissions
        try:
            with open(filepath, 'a') as f:
                pass
            os.remove(filepath)  # Remove the test file
        ...
        try:
            ...
            logger.error(f"Error building PDF content: {str(e)}")
            raise
