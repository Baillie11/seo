...
def create_pdf_report(seo_data, categories, filename=None):
    ...
    if "Technical SEO" in categories:
        ...
        c.drawString(50, y_position, f"Page Load Time: {seo_data.get('Technical SEO', {}).get('Page Load Time', 'Not available')}")
        y_position -= 20
        c.drawString(50, y_position, f"Rating: {seo_data.get('Technical SEO', {}).get('Load Time Rating', 'Not rated')}")
        y_position -= 30
    ...
    if "On-Page SEO" in categories:
        ...
        y_position -= 20
        c.setFont("Helvetica", 12)
        title_tag = seo_data.get('On-Page SEO', {}).get('Title Tag', 'No Title Found')
        ...
        y_position -= 20
        c.drawString(50, y_position, f"Title Length Status: {seo_data.get('On-Page SEO', {}).get('Title Length Status', 'Not checked')}")
        y_position -= 20
        c.drawString(50, y_position, f"Keyword Presence: {seo_data.get('On-Page SEO', {}).get('Keyword Presence', 'Not checked')}")
        y_position -= 30  # Space before next section if any

    c.save()
    print("Report Created")  # Debugging print statement
    return full_path
